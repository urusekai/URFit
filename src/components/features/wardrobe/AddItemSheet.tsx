...
/**
 * 옷장에 새 옷을 등록하는 모달.
 * 온보딩 3단계의 "옷 정보 등록"과 동일한 구성(사진 업로드 → 카테고리·소재·핏·사이즈·치수)이며,
 * 저장 시 clothes 테이블 + 스토리지에 실제로 기록한다(서버 액션 saveClosetItem).
 */
export function AddItemSheet({ onClose, onAdd }: AddItemSheetProps) {
  ...
  /** 카테고리를 바꾸면 그 카테고리의 핏·사이즈·치수 기본값으로 초기화. (온보딩과 동일) */
